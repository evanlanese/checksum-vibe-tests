...
test.describe("Create, edit and delete a new product", () => {
  ...
  test(
    ...
    async ({ page, vs }) => {
      ...
      await expect(
        toast,
        "Toast should show product created message"
      ).toHaveText("Product created successfully");

      // Step 6: Verify redirected to home page and the new product appears in the list
      await expect(page, "Should be redirected to home page").toHaveURL(
        /index\.html|\/$/
      );
      const productItem = getProductItem(page, vs.productName);
      await expect(
        productItem,
        "Newly created product should appear in the list"
      ).toBeVisible();
      ...
      await expect(
        page.locator("#pageTitle"),
        "Page title should show Edit Product"
      ).toHaveText("Edit Product");
      await expect(
        page.locator("#productName"),
        "Product name field should be pre-filled"
      ).toHaveValue(vs.productName);
      await expect(
        page.locator("#productDescription"),
        "Product description field should be pre-filled"
      ).toHaveValue(vs.productDescription);
      ...
      await checksumAI("Update the product description", async () => {
        await page
          ...
          .fill(vs.updatedProductDescription);
      });

      // Step 11: Click 'Save Product' button
      await checksumAI(
        "Click Save Product button to save updates",
        async () => {
          await page.locator("#saveBtn").click();
        }
      );
      ...
      await expect(
        toast,
        "Toast notification should be visible after update"
      ).toBeVisible();
      await expect(
        toast,
        "Toast should show success message after update"
      ).toHaveText("Product created successfully");

      // Step 13: Verify redirected to home page and product shows updated name
      await expect(
        page,
        "Should be redirected to home page after update"
      ...
      await checksumAI(
        "Click Delete button on the updated product",
        async () => {
      ...
      await expect(
        toast,
        "Toast notification should be visible after deletion"
      ).toBeVisible();
      await expect(
        toast,
        "Toast should show product deleted message"
      ).toHaveText("Product deleted successfully");

      // Step 18: Verify the product is removed from the list
      await expect(
        updatedProductItem,
        "Deleted product should no longer appear in the list"
      ).not.toBeVisible();
    }
  );
});
